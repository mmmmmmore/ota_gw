...
static void register_sock(int sock, msg_role_t role) {
    lock();
    // If we already have an OTA server socket, close it
...
static void gw_keep_alive_task(void *pvParameters) {
    ...
    while (1) {
        ...
        for (int i = 0; i < MAX_SOCKS; i++) {
            int sock = socks[i];
            if (sock > 0) {
                safe_send(sock, keep_alive_msg, strlen(keep_alive_msg));
            }
        }
    }
}

/* ---------------- Framing + parse per-connection ---------------- */
static void process_streaming_json(int client_sock, msg_role_t role, const char *chunk, int len) {
    if (len <= 0) return;
    ...
    ESP_LOGI(TAG, "Received %d bytes from client %d: %s", len, client_sock, si->rx_buffer);
    ...
    while (1) {
        ...
        if (msg_len >= RX_BUF_SIZE) { // safety
            ESP_LOGW(TAG, "Single JSON too long (%d), dropping", msg_len);
            start = newline + 1;
            continue;
        }
        ...
        if (root) {
            ...
            if (msg_type && msg_type->valuestring) {
                if (strcmp(msg_type->valuestring, "keep_alive_ack") == 0) {
                    ESP_LOGI(TAG, "[%s] Rx keep_alive_ack from sock: %d",
                             (role == ROLE_CLIENT ? "CLIENT" : "OTA"), client_sock);
                    update_last_seen(client_sock);
                } else {
                    ESP_LOGI(TAG, "[%s] Rx msg_type = %s from sock %d",
                             (role == ROLE_CLIENT ? "CLIENT" : "OTA"),
                             msg_type->valuestring, client_sock);
                    msg_handler_process(client_sock, json_str, role);
                    update_last_seen(client_sock);
                }
            } else {
                ESP_LOGW(TAG, "JSON missing msg_type from sock %d : %s", client_sock, json_str);
                update_last_seen(client_sock);
            }
            cJSON_Delete(root);
        } else {
            ESP_LOGW(TAG, "Failed to parse JSON from sock %d : raw=%s", client_sock, json_str);
            update_last_seen(client_sock);
        }
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (len < 0) {
                ...
                // per-role framing/parse
                process_streaming_json(client_sock, role, chunk, len);
            }
        }
